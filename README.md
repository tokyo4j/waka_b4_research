...
## Title(temp)
プロセス状態のログ分析を利用したスケジューラ改善手法の提案

## Abstract
オペレーティングシステムにおけるスケジューラはクラッシュやデッドロックのような明確な障害を伴わないことが多く,
意図した通りに CPU 時間の割り当てが行われているかどうかを確認することが難しい.
本研究ではオペレーティングシステム xv6 を題材とし, プロセス状態について収集したログを元に,
スケジューラの脆弱性発見からFairness 改善を行うまでの一連のプロセスを提案する.
スケジューリングイベントが発生した際のプロセス・コア状態を記録することで,
各コアでどのプロセスが実行されているかを時系列で確認することが可能になる.
